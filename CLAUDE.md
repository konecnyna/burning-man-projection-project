...
| Camera TCC identity (app bundle) | `deploy/build-app-bundle.sh` |
| One-time camera grant | `deploy/grant-camera.sh` |
...
- **The camera grant belongs to the app bundle, not to python.** macOS
  attributes camera access to the *responsible process*. Launched directly by
  launchd that is `com.apple.python3` — no TCC grant, no
  `NSCameraUsageDescription`, denied silently, never prompts. OpenCV says "not
  authorized, status 0", which means *never asked*, not denied.
  The LaunchAgent therefore launches `deploy/ATLANTIS-Kiosk.app` via
  `/usr/bin/open -W`. It is an `osacompile` applet (generated by
  `deploy/build-app-bundle.sh`) that runs `start-atlantis.sh` as a **child**,
  so python inherits the app's identity. **The LaunchAgent path has a working
  camera** — that is the supported one.
  Three things will waste your time here: the bundle's main executable must be
  a **Mach-O** (LaunchServices refuses script-based bundles with `-10669`, so
  they get no identity at all); the bundle must be launched **via `open`**, not
  by spawning a path inside it; and it must **not** `exec`, which would discard
  the identity. Any rebuild changes the ad-hoc cdhash and **voids the grant** —
  `build-app-bundle.sh` is idempotent for exactly this reason, and
  `deploy/grant-camera.sh` re-establishes it with one click on the console.
  Never rename the bundle to `ATLANTIS.app`: that path is permanently poisoned
  in LaunchServices on the deployment box.
  `kiosk-ctl.sh console` still exists for SSH debugging, but it only *borrows*
  Terminal.app's grant for one run and proves nothing about boot.
  `kiosk-ctl.sh status` reports whether the camera actually opened. See
  [DEPLOYMENT.md §6](DEPLOYMENT.md#6-camera-permissions).
